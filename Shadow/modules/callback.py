...
from Shadow.database import JAAN
...
async def cb_handler(_, query: CallbackQuery):
    if query.data == "HELP":
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_JAAN = JAAN.find_one({"chat_id": query.message.chat.id})
            if not is_dil:
                await query.edit_message_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ.**")
            if is_JAAN:
                JAAN.delete_one({"chat_id": query.message.chat.id})
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_JAAN = JAAN.find_one({"chat_id": query.message.chat.id})
            if not is_JAAN:
                JAAN.insert_one({"chat_id": query.message.chat.id})
                await query.edit_message_text(
