...
async def executor(client: Shadow, message: Message):
...
@Shadow.on_cb("runtime")
async def runtime_func_cq(_, cq):
    runtime = cq.data.split(None, 1)[1]
    await cq.answer(runtime, show_alert=True)


@Shadow.on_cb("forceclose")
...
async def shellrunner(client: Shadow, message: Message):
    if len(message.command) < 2:
